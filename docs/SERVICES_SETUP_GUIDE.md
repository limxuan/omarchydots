...
2. **Automatic Extension Loading**: Unpacked extensions (SponsorBlock, Vimium C, Dark Reader, FoxyProxy, Bitwarden, Keyboard Shortcuts) are downloaded to `~/.config/net.imput.helium/unpacked-extensions/` and passed directly via `--load-extension` CLI flags in the wrapper script. No manual UI loading is required.

### Important Quirks:
- **Already Running Instances**: Chromium ignores `--load-extension` flags if Helium is already running in the background. If extensions fail to load, run `pkill helium` and launch it again.
- **Tilde Expansion**: Chromium flags do not expand `~` automatically, so full absolute paths are generated by `~/.local/bin/helium-browser`.
